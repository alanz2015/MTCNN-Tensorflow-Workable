## Description
This work is forked from already existed MTCNN-Tensor project in github, apply necessary modification to make the MTCNN Tensorflow can work in both training and inference.

## Prerequisites
1. You need CUDA-compatible GPUs to train the model. I trained PNet, RNet, and ONet w/ 2 x V100 for 3 days on Wider_Face, and LFW for 3 days.
...
7. Run `train_PNet.py` to perform train PNet w/ the generated data.
...
12. Run `train_RNet.py`
13. After training **RNet**, run `gen_hard_example4ONet.py` to generate training data(Face Detection Part) for **ONet**.
